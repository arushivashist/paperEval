...
# Check the number of respondents in each condition
# Cond = 1 and Cond = 2: "NoBrand" condition (item category only condition)
# Condition 3: Brand condition with the brand names group A
# Condition 4: Brand condition with the brand names group B
dat %>% 
  group_by(Cond) %>% 
  summarise(n())
...
##================================================================================
## 2 (Level: NoBrand vs. Brand; between-subject) X 2 (Product Type: Utilitarian vs. Hedonic; within-subject) mixed ANOVA
## The pre-registered hypothesis was that "Level X Product Type interaction is statistically significant". 
...
contrast.condition <- emmeans(model1, pairwise ~ Condition|ProductType, adjust="none")
contrast.condition$contrasts

contrast.producttype <- emmeans(model1, pairwise ~ ProductType|Condition, adjust="none")
contrast.producttype$contrasts
